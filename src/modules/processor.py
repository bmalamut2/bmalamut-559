...
def get_performance(binding_score, binding, binding_pred):
    accuracy = accuracy_score(binding, binding_pred)
    precision = precision_score(binding, binding_pred)
    recall = recall_score(binding, binding_pred)
    f1 = f1_score(binding, binding_pred)
    auc = roc_auc_score(np.array(binding), np.array(binding_score)) if len(np.unique(np.array(binding))) != 1 else -1
...
def dataRepresentationBlosum62WithMHCb(X_data):
    ...
    m_cdr3 = enccodeListBlosumMaxLen(X_data.CDR3b, encoding, 40)
    m_cdr3 = m_cdr3.reshape(len(m_cdr3), 380)

    m_epitope = enccodeListBlosumMaxLen(X_data.epitope, encoding, 50)
...
def dataRepresentationBlosum62WithoutMHCb(X_data):
    ...
    m_cdr3 = enccodeListBlosumMaxLen(X_data.CDR3b, encoding, 40)
    m_cdr3 = m_cdr3.reshape(len(m_cdr3), 800)

    m_epitope = enccodeListBlosumMaxLen(X_data.epitope, encoding, 50)
    m_epitope = m_epitope.reshape(len(m_epitope), 1000)
...
def dataRepresentationBlosum62WithoutMHCa(X_data):
    ...
    m_cdr3 = enccodeListBlosumMaxLen(X_data.CDR3a, encoding, 40)
    m_cdr3 = m_cdr3.reshape(len(m_cdr3), 380)
